...
public class FileMatching {
    ...
    private static void mergeFilesData(){ 
        ...
        Log log = new Log("src/filematching/log.txt","src/filematching/intrans.ser");
    ...
    private static void openInNewMaster(){
        try{
            inNewMaster = new ObjectInputStream(Files.newInputStream(Paths.get("src/filematching/newmast.ser")));
        }
        catch(IOException e){
    ...
    private static void openInOldMaster(){
        try{
            inOldMaster = new ObjectInputStream(Files.newInputStream(Paths.get("src/filematching/oldmast.ser")));
        }
        catch(IOException e){
            System.err.println("Error Opening File. Terminating.");
            System.exit(1);
    ...
    private static void openInTransactions(){
        try{
            inTransaction = new ObjectInputStream(Files.newInputStream(Paths.get("src/filematching/intrans.ser")));
        }
        catch (IOException e){
            System.err.println("Error Opening File. Terminating.");
            System.exit(1);
    ...
    private static void openOutNewMaster(){
        try{
            outNewMaster = new ObjectOutputStream(Files.newOutputStream(Paths.get("src/filematching/newmast.ser")));
        }
